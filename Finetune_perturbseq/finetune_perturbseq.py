...
def split_gene_ids(gene_ids, seed_value=42):
    rng = np.random.RandomState(seed_value)
    gene_ids = np.array(sorted(set(gene_ids)))
    rng.shuffle(gene_ids)
    n = len(gene_ids)
    n_train = int(n * 0.8)
    n_val = int(n * 0.1)
    train_ids = gene_ids[:n_train].tolist()
    val_ids = gene_ids[n_train : n_train + n_val].tolist()
    test_ids = gene_ids[n_train + n_val :].tolist()
...
def main():
    ...
    hvg_path = os.path.join(args.de_result_dir, "hvg_geneids_5000.pkl")
    with open(hvg_path, "rb") as f:
        hvg_geneids = pickle.load(f)

    with h5py.File(h5_data_path, "r") as f:
        all_pert_gene_ids = f["pertubed_gene_id"][:].tolist()

    train_ids, val_ids, test_ids = split_gene_ids(all_pert_gene_ids, seed_value=42)
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=4, sampler=train_sampler, collate_fn=collate_fn, num_workers=0, pin_memory=True
    )
    val_loader = torch.utils.data.DataLoader(
        val_dataset, batch_size=4, sampler=val_sampler, collate_fn=collate_fn, num_workers=0, pin_memory=True
    )
    test_loader = torch.utils.data.DataLoader(
        test_dataset, batch_size=4, sampler=test_sampler, collate_fn=collate_fn, num_workers=0, pin_memory=True
    )
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}", disable=(args.local_rank != 0)):
